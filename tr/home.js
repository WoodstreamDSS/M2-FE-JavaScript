...
(function($) {
  ...
  $(document).ready(function() {
    ...
    if ($('#ws-reviews').length > 0) { var dtLoggedOutZoom = new woodstream.z3('ws-reviews', true); };
    blockSliders.push(new woodstream.BlockSlider('article-slider'));
    blockSliders.push(new woodstream.BlockSlider('learning-slider'));
    blockSliders.push(new woodstream.BlockSlider('youtube-slider'));
    blockSliders[1].hide();
    blockSliders[2].hide();

    $('.home-tabs .tab-panel[aria-hidden="true"]').find('a.cta').attr('tabindex',-1);
  });
