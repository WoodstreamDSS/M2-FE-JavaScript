require(['jquery'], function ($) {
  // let bannerIndex = localStorage.getItem('bannerIndex') || 0;
  // let banners = $('vp-homepage-2022 .hero-banner>div');

  // if (bannerIndex == null) {
  //   localStorage.setItem('bannerIndex', 0);
  // } else {
  //   bannerIndex ++;
  //   if (bannerIndex > banners.length - 1) { bannerIndex = 0; };
  //   localStorage.setItem('bannerIndex', bannerIndex);
  // }
  // banners.eq(bannerIndex).show();

  woodstream.MBEqualHeight = function (selector, breakpoint) {
    ...
    this.setHeight = function () {
      ...
      if (docWidth > this.breakpoint) {
        ...
        this.collection.each(function () {
          ...
          } // if it's taller than the current maximum height, set this as the new maximum height
        });
    ...
    $(window).on(
      'resize',
      function () {
        // listen for window resize, trigger recalculation (might want to implement a debounce at some point)
        this.collection = $(selector);
        this.setHeight();
      }.bind(this)
    );
    ...
    setTimeout(
      ...
      100
    ); // wait a short amount of time before the initial resize, to be sure images are loaded. If server is proving slower than this, increase the wait time
  };
  ...
  woodstream.ReviewSlider = class {
    ...
    updateAria() {
      this.mobile
        ? this.buttons.attr('aria-hidden', true)
        : this.buttons.attr('aria-hidden', false);

      if (this.idx < 0) {
        this.idx = this.reviews().length - 1;
      }
      if (this.idx > this.reviews().length - 1) {
        this.idx = 0;
      }
      this.reviews().attr('aria-selected', false);
      this.wrap
        .find(`.review[data-idx="${this.idx}"]`)
    ...
    init() {
      ...
      this.circles.on('click', (e) => {
        this.circleHandler(e.currentTarget);
      });
  ...
  const reviewTitlesEH = new woodstream.MBEqualHeight(
    '.mb-homepage-2022 .reviews .mb-reviews .review span.review-title',
    0
  );
  const reviewProductsEH = new woodstream.MBEqualHeight(
    '.mb-homepage-2022 .reviews .mb-reviews .review span.review-product',
    0
  );
  const reviewTextEH = new woodstream.MBEqualHeight(
    '.mb-homepage-2022 .reviews .mb-reviews .review .review-text',
    0
  );
